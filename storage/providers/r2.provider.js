 ...
 * R2 uses S3-compatible API, so implementation mirrors S3 provider.
 *
 * No direct R2 SDK access outside this provider.
 */
...
export class R2StorageProvider extends StorageProviderInterface {
  constructor(config = {}) {
    super({ ...DEFAULT_CONFIG, ...config })
    this.type = STORAGE_PROVIDER_TYPES.R2
  ...
  async initialize() {
    if (!this.accountId) {
      throw new StorageProviderError('R2 accountId is required', this.type)
    }
    if (!this.bucket) {
      throw new StorageProviderError('R2 bucket is required', this.type)
    }

    this.client = await this.createClient()
    this.initialized = true

    return { initialized: true, bucket: this.bucket, accountId: this.accountId }
  }
  ...
  async download(assetId, options = {}) {
    ...
    try {
      ...
      throw new StorageProviderError(
        `R2 download failed: ${error.message}`,
        this.type,
        { assetId }
      )
    }
  ...
  async stream(assetId, options = {}) {
    ...
    try {
      ...
      throw new StorageProviderError(
        `R2 stream failed: ${error.message}`,
        this.type,
        { assetId }
      )
    }
  ...
  async delete(assetId, options = {}) {
    ...
    try {
      ...
      throw new StorageProviderError(
        `R2 delete failed: ${error.message}`,
        this.type,
        { assetId }
      )
    }
  ...
  async move(assetId, destPath, options = {}) {
    ...
    const destKey = this.buildKey(destPath, this.path.basename(srcKey))
  ...
  async list(prefix, options = {}) {
    ...
    try {
      ...
      return (response.Contents || []).map((item) => ({
        name: this.path.basename(item.Key),
        path: item.Key,
        size: item.Size,
  ...
  async createFolder(folderPath, options = {}) {
    ...
    try {
      ...
      throw new StorageProviderError(
        `R2 createFolder failed: ${error.message}`,
        this.type,
        { folderPath }
      )
    }
  ...
  async deleteFolder(folderPath, options = {}) {
    ...
    try {
      ...
      throw new StorageProviderError(
        `R2 deleteFolder failed: ${error.message}`,
        this.type,
        { folderPath }
      )
    }
  ...
  async generateSignedUrl(assetId, options = {}) {
    ...
    try {
      ...
      throw new StorageProviderError(
        `R2 signed URL failed: ${error.message}`,
        this.type,
        { assetId }
      )
    }
  ...
  async getMetadata(assetId) {
    ...
    try {
      ...
      throw new StorageProviderError(
        `R2 getMetadata failed: ${error.message}`,
        this.type,
        { assetId }
      )
    }
  ...
  async setMetadata(assetId, metadata) {
    ...
    try {
      ...
      throw new StorageProviderError(
        `R2 setMetadata failed: ${error.message}`,
        this.type,
        { assetId }
      )
    }
  ...
  async getChecksum(assetId, options = {}) {
    ...
    try {
      ...
      throw new StorageProviderError(
        `R2 getChecksum failed: ${error.message}`,
        this.type,
        { assetId }
      )
    }
